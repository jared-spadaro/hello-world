 ...
 * This program takes a filesystem image and a path to a file
 * and copies the file to a given destination path, or stdout
 */
...
typedef struct fs_info {
   ...
   char *destpath;  /* path to copy to */
} fs_info;

typedef struct part_info {
  int *start;       /* first sector of partition */
...
void Usage () {
   printf("usage: minget  [ -v ] [ -p num [ -s num ] ] imagefile" 
    " srcpath [ dstpath ]\n");
   printf("Options:\n"
...
void RetrieveFile (struct inode_t *inode, struct fs_info *fs, int zone_ndx, 
 int type, FILE *stream, int zonesize, char *name, struct part_info *part) {
   int i, path_len, offset;
   unsigned char temp;
   FILE *dest;
   char *path;  
   if (fs->part == -1) {
   ...
   if (fs->destpath == NULL) {
      ...
      path_len = strlen(fs->destpath);
      if (fs->destpath[path_len-1] != '/') {
         strcat(fs->destpath, "/");
      }
      path = strcat(fs->destpath, name);
      dest = OpenFS(path, "w+");
      if (dest == NULL) {
         perror("fopen");
         exit(1);
   ...
   if (type == DIR_FILE) {
      fprintf(stderr, "Error: specified file is not regular\n");
      exit(1);
   }
...
void TraverseDir (FILE *stream, struct inode_t *inode, struct part_info *part, 
 ...
 struct fs_info *fs, char *name) {
   ...
   if (type != DIR_FILE || parsed_path[path_ndx] == NULL) {
      RetrieveFile(inode, fs, zone_ndx, type, stream, zonesize, name, part);   
      exit(0);
   }
   /* from this point forward, we know that we are traversing a dir */
   SafeSeek(stream, zonesize * inode->zone[0], SEEK_SET);
   for (i = 0; i < num_subdirs; i++) {
      ...
      if(!strcmp(dir->name, parsed_path[path_ndx])) {
         Read_iNode(stream, part, super, new_inode, dir->inode, fs);
         TraverseDir(stream, new_inode, part, super, parsed_path, 
          ++path_ndx, zone_ndx, fs, dir->name);
      }
   } 
...
void Read_Super (FILE *stream, struct superblock_t *super, 
 ...
 struct fs_info *fs) {
   if (fs->part == -1) {
      ...
      Read_iNode(stream, NULL, super, inode, 1, fs);
   }
   else if (fs->subpart == -1) {
      ReadSuperBlock(stream, super, part);
      Read_iNode(stream, part, super, inode, 1, fs);
   }
   else {
      ReadSuperBlock(stream, super, subpart);
      Read_iNode(stream, subpart, super, inode, 1, fs);
   }
   if (fs->verbose) {
...
void FollowPath (FILE *stream, struct fs_info *fs, struct inode_t *inode,
 struct superblock_t *super, struct part_info *part) {
   ...
   /* we weren't given a regular file to get */
   else {
      fprintf(stderr, "Error: must give path to regular file to retrieve\n");
      exit(1);
   }
}
...
int main (int argc, char **argv) {
   ...
   struct part_info *part = malloc(sizeof(part_info));
   ...
   Initialize(fs_info, part, subpart_info, argc, argv);
   stream = OpenFS(fs_info->image, "r");
   Goto_Partition(fs_info, part, subpart_info, stream);
   Read_Super(stream, super, root_inode, part, subpart_info, fs_info);
   FollowPath(stream, fs_info, root_inode, super, part);
