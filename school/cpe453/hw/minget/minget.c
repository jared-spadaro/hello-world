...
typedef struct fs_info {
   ...
   char *path;      /* the path in the filesystem image that we will be listing */
} fs_info;
 ...
 */
void SeekPartition(struct fs_info *fs_info, struct part_info *part_info, FILE *stream, int offset) {
...
void PrintFileMetadata (FILE *stream, struct inode_t *inode, int zone_ndx,
 struct part_info *part, struct superblock_t *super, struct fs_info *fs) {
   ...
   if (type == REG_FILE) {
      ...
      SafeSeek(stream, zonesize * inode->zone[zone_ndx], SEEK_SET);
      printf("%s:\n", fs->path);
      ...
      for (i = 0; i < num_subdirs; i++) {
         ...
         FormatMode(formatted_mode, new_inode->mode, QueryMode(new_inode->mode));
         printf("%s%9d %s\n", formatted_mode, new_inode->size, dir_array[i]->name);
      }
...
void RetrieveFile (struct inode_t *inode, struct fs_info *fs, 
 int zone_ndx, int type) {
   if (type == DIR_FILE) {
      fprintf(stderr, "Error: specified file is not regular\n");
      exit(1);
   }
   printf("printing data of file with size %d\n", inode->size);
}
...
void TraverseDir (FILE *stream, struct inode_t *inode, struct part_info *part, 
 ...
 struct fs_info *fs) {
   ...
   if (type != DIR_FILE || parsed_path[path_ndx] == NULL) {
      ...
      RetrieveFile(inode, fs, zone_ndx, type);   
      exit(0);
   }
...
void Read_Super (FILE *stream, struct superblock_t *super, struct inode_t *inode,
 struct part_info *part, struct part_info *subpart, struct fs_info *fs) {
   if (fs->part == -1) {
