 ...
 * Lab 5
 * 11/29/15
 * This program emulates and simulates mips assembly code
 * and emulates a correlating branch predictor.
 */
...
public class lab5 {
   ...
   //lab5 variables
   private double correct;     //number of correct branch predictions
   private double total;       //total number of branches in code (taken and not taken)
   private int ghr;           //global history register (2/4/8 bits in length)
   private int[] predictor;   //array of predictions (indexed into by ghr)   
   private int prediction;    //what is our prediction (placed in predictor array) 
   private int actual;        //is the branch actually taken (this will get shifted into ghr)
   ...
   public void printPipe() {
      ...
      System.out.printf("%-8d%-8s%-8s%-8s%-8s\n", pc, pipeline[0], pipeline[1], 
   ...
   public void predictBranch (boolean isTaken) {
     //if we've entered this method, we must be at a branch instruction
     int temp;
     
     if(ghr_size == 2) {
        temp = ghr & 0x3;
        prediction = predictor[temp];
        if(isTaken) {  //increment predictor at ghr index
           if(predictor[temp] < 3) predictor[temp]++;
        }
        else {   //decrement predictor at ghr index
           if(predictor[temp] > 0) predictor[temp]--;
        }
     }
     else if (ghr_size == 4) {
        temp = ghr & 0xF;
        ...
        System.out.println("Wrong ghr size given");
        System.exit(0);
     }
     ghr <<= 1;                        //shift ghr to left to prepare to update
     ghr &= 0x7FFFFFFF;                //clear sign bit just in case 
     if(isTaken) {                 //place 1 in LSB if branch is taken, else leave it as 0 
        ghr |= 1;
     }

     if((prediction > 1 && isTaken == true) || (prediction < 2 && isTaken == false)) correct++;

     total++;
   }
     
   public void outputBranchAccuracy () {
     if(total == 0) System.out.println("No branches have been executed yet.\n");
     else {
        //System.out.println("accuracy " + ((correct/total)*100) + "%" + " (" + correct + 
        //" correct predictions, " + total + " predictions)\n");
        System.out.printf("accuracy %-6.2f",(correct/total)*100);
        System.out.print("% (");
        System.out.printf("%-5.0f correct predictions, %-5.0f predictions)\n\n",correct,total);
     }
   }
   ...
   public void shiftTest () {
      byte x = 1;
      System.out.println("unchanged number = " + x);
      for(int i = 0; i<8; i++) {
         System.out.println("shifted number = " + (x << i));
      }
   }
          

   public void outputToCSV () throws FileNotFoundException {
   ...
   public void scanFileNames (String[] args) throws FileNotFoundException {
      ...
      if(args.length == 3) {
         ...
         if(ghr_size != 2 && ghr_size != 4 && ghr_size !=8) {
            System.out.println("Invalid ghr size");
            System.exit(0);
         }
      }
      else {
    ...
    public void decode(Instruction instr, String args) {
      ...
      while (scan.hasNext()) {
         ...
         if (args.contains("($")) { //lw or sw
            index = args.indexOf('(');
            immediate = args.substring(0, index);
            register[i] = args.substring(index + 1, args.length() - 1);
   ...
   public void endProgram() {
      if (end > 0) {
         ...
         System.out.printf("CPI = %1$-6.3f Cycles = %2$-6d Instructions = %3$-6d\n", cycles/instructions, 
          (int)cycles, (int)instructions);
         end = -1;
         System.out.println();
      }  
   }
   ...
   public void interpretCommand (String cmd) throws FileNotFoundException {
      DecimalFormat numFormat = new DecimalFormat("#.000");
      int i,num1,num2;
      if(cmd.equals("h")) {
         System.out.println("");
         System.out.println("h = show help");
         System.out.println("b = output branch accuracy");
         System.out.println("o = output coordinates to csv file");
         ...
         if(cmd.contains("s")) {
            ...
            if (end != -1)
               printPipe();
            System.out.println("       " + num1 + " cycle(s) executed");
         }
